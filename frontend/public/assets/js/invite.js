...
document.addEventListener('DOMContentLoaded', async () => {
  spawnParticles();
  const slug = getSlug();
  if (!slug) { showError('No invitation ID provided.', 'Missing Link'); return; }
...
async function loadInvitation(slug) {
  try {
    const res = await fetch(`${API_BASE_URL}/api/invitations/${encodeURIComponent(slug)}`);
...
async function submitRsvp(e) {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/invitations/${invitation.slug}/rsvp`, {
      method: 'POST',
    ...
    successEl.querySelector('.success-icon').textContent =
      attending === 'yes' ? '🎉' : '💌';
  } catch (err) {
